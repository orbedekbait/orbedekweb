...
document.querySelector('.inquiry-form')?.addEventListener('submit',event=>{event.preventDefault();const data=new FormData(event.currentTarget);const text=`פנייה חדשה מאתר אור בדק בית%0Aשם: ${encodeURIComponent(data.get('name'))}%0Aטלפון: ${encodeURIComponent(data.get('phone'))}%0Aסוג נכס: ${encodeURIComponent(data.get('property')||'לא צוין')}%0Aסוג שירות: ${encodeURIComponent(data.get('service')||'לא צוין')}`;window.open(`https://wa.me/972523199403?text=${text}`,'_blank','noopener')});
