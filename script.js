...
const elfsightGoogleReviews=document.querySelector('.elfsight-app-317ec25e-880d-4157-bd2a-60622119a50e');
if(elfsightGoogleReviews){
  let attempts=0;
  const customizeElfsightHeader=()=>{
    attempts+=1;
    const host=elfsightGoogleReviews.querySelector('.es-embed-root');
    const shadow=host?.shadowRoot;
    if(!shadow){
      if(attempts>=150)window.clearInterval(elfsightHeaderTimer);
      return;
    }
  ...
  const elfsightHeaderTimer=window.setInterval(customizeElfsightHeader,200);
  customizeElfsightHeader();
}
